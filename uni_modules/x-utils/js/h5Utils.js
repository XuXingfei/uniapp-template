...
// 微信公众号授权获取 code
export const getWxAuthCode = (state = '', scope = 'snsapi_base') => {
    /*
     以snsapi_base为scope发起的网页授权，是用来获取进入页面的用户的openid的，并且是静默授权并自动跳转到回调页的。用户感知的就是直接进入了回调页（往往是业务页面）
     以snsapi_userinfo为scope发起的网页授权，是用来获取用户的基本信息的。但这种授权需要用户手动同意，并且由于用户同意过，所以无须关注，就可在授权后获取该用户的基本信息。
    */
    let redirect_uri = encodeURIComponent(window.location.href);
    let authUrl = `http://open.weixin.qq.com/connect/oauth2/authorize
...
// 支付宝生活号授权获取 code
export const getZfbAuthCode = (scope = 'auth_base') => {
    /*
     auth_base(静默授权)：静默授权，用户无需点击确认授权，默认返回 auth_code，该授权码不支持获取用户信息。
     auth_user(主动授权)：首次授权需要用户手动点击同意，用户同意后，返回 auth_code；商家需要考虑用户拒绝授权的情况并进行相应容错。如果授权关系依旧存在，下次进入页面时也会静默授权。
    */
    return new Promise((resolve, reject) => {
        ap.getAuthCode({
